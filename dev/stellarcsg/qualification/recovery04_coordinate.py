...
def invoke(executable: Path, bank: Path, wistell: Path | None, dataset: str, timeout: float) -> dict:
    command = [str(executable), "--replay", str(bank), "--skip-reference"]
    if wistell:
    ...
    try:
        ...
        return {"command": command, "returncode": None, "stderr": error.stderr,
                "records": [], "malformed_stdout_lines": 0, "summary": None,
                "state": "BLOCKED", "reason": "timeout"}
    records = []
    malformed = 0
    ...
    return {"command": command, "returncode": completed.returncode,
            ...
            "state": "PASS" if completed.returncode == 0 else "BLOCKED"}


def durable_write(path: Path, value: dict) -> None:
...
def cross_version(archive: dict, latest: dict) -> dict:
    ...
    for query_id in sorted(set(older) & set(newer)):
        ...
        if a.get("state") != b.get("state") or a.get("candidate_state") != b.get("candidate_state"):
            mismatches.append(query_id)
        elif a.get("candidate_found") != b.get("candidate_found"):
            mismatches.append(query_id)
        elif a.get("candidate_found") and abs(a["candidate_distance"] - b["candidate_distance"]) > 2e-8:
            mismatches.append(query_id)
...
def metamorphic(run: dict) -> dict:
    ...
    return {"direction_scaling_a09_equals_a10": same_distance("a09", "a10"),
...
def main() -> int:
    ...
    receipt = {"schema": "recovery04-frozen-bank-v1", "created_utc": dt.datetime.now(dt.timezone.utc).isoformat(),
               ...
               "executables": {"archive_sha256": file_hash(args.archive), "latest_sha256": file_hash(args.latest)},
    ...
    for name, executable in (("archive", args.archive), ("latest", args.latest)):
        for repetition in range(7):
            item = invoke(executable, args.bank, args.wistell_h5, args.wistell_dataset, args.timeout_seconds)
            item["repetition"] = repetition
            item["metamorphic"] = metamorphic(item)
            receipt[name].append(item)
            durable_write(args.out, receipt)
