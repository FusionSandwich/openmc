  ...
  : Surface(surf_node)
{
  if (!check_for_node(surf_node, "data_file")) {
    fatal_error(fmt::format(
      "Periodic-spline surface {} requires data_file", id_));
  }
  if (!check_for_node(surf_node, "dataset")) {
    fatal_error(fmt::format(
      "Periodic-spline surface {} requires dataset", id_));
  }
  ...
  if (solver_ != "fast" && solver_ != "reference") {
    fatal_error(fmt::format(
      "Periodic-spline surface {} supports solver='fast' or 'reference'; "
      "received '{}'", id_, solver_));
  }
  ...
  Position r, Direction u, bool coincident) const
{
  ...
  stellarcsg::FastDistanceOptions options;
  options.fallback_to_reference = true;
  options.fallback_options.initial_subdivisions = 96;
  options.fallback_options.max_refinement_levels = 7;
  options.fallback_options.require_refinement_stability = true;
  const auto result = surface_->distance_fast(
    to_vec3(r), to_vec3(u), coincident, options);
  return result.found ? result.distance : INFTY;
}
