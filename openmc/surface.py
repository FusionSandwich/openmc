...
class Surface(IDManagerMixin, ABC):
    ...
    def from_xml_element(elem):
        ...
        if surf_type == 'periodic-spline':
            kwargs.update({
                ...
                'solver': get_text(elem, 'solver', 'fast'),
            })
        else:
    ...
    def from_hdf5(group):
        ...
        coeffs = () if surf_type == 'periodic-spline' else group['coefficients'][...]
        cls = _SURFACE_CLASSES[surf_type]
        if surf_type == 'periodic-spline':
            def _read_text(name, default=None):
                if name not in group:
                    return default
                value = group[name][()]
                return value.decode() if isinstance(value, bytes) else str(value)
            return cls(
                data_file=_read_text('data_file'),
                dataset=_read_text('dataset'),
                content_id=_read_text('content_id'),
                solver=_read_text('solver', 'fast'),
                **kwargs,
            )
...
class PeriodicSplineSurface(Surface):
    r"""A native smooth periodic radial-spline stellarator surface.

    The surface coefficients are stored in a versioned HDF5 group generated by
    the ``stellarcsg`` preprocessing package. The compiled OpenMC executable
    must be configured with ``OPENMC_USE_STELLARATOR_CSG=ON``.

    Parameters
    ----------
    data_file : path-like
        HDF5 file containing the frozen surface coefficients.
    dataset : str
        Absolute HDF5 group path, such as ``/surfaces/plasma``.
    content_id : str
        SHA-256-derived content identifier expected in the HDF5 group.
    solver : {'fast', 'reference'}, optional
        Ray-distance algorithm. ``fast`` uses the bounded-work production
        search with a correctness fallback; ``reference`` uses the conservative
        full-interval oracle.
    kwargs
    ...
    def __init__(self, data_file, dataset, content_id, solver='fast', *args,
                 **kwargs):
        kwargs = _future_kwargs_warning_helper(type(self), *args, **kwargs)
        super().__init__(**kwargs)
        if not isinstance(data_file, (str, bytes)) and not hasattr(data_file, '__fspath__'):
            raise TypeError('data_file must be path-like')
        if not isinstance(dataset, str) or not dataset.startswith('/'):
        ...
        check_value('periodic-spline solver', solver, ('fast', 'reference'))
    ...
    def _load_reference_surface(self):
        if self._reference_surface is None:
            try:
                from stellarcsg import read_surface
            except ImportError as exc:
                raise RuntimeError(
                    'Python-side evaluation of PeriodicSplineSurface requires '
                    'the dev/stellarcsg Python package'
                ) from exc
            self._reference_surface = read_surface(
                self.data_file, self.dataset,
                expected_content_id=self.content_id)
        return self._reference_surface

    def evaluate(self, point):
        return float(self._load_reference_surface().evaluate(point))

    def bounding_box(self, side):
        if side == '+':
            return BoundingBox.infinite()
        if self._cached_bounding_box is None:
            surface = self._load_reference_surface()
            radius_max = float(np.max(surface.radius_coefficients))
            radial_extent = float(
                np.max(np.abs(surface.axis_r_coefficients)) + radius_max)
            z_min = float(np.min(surface.axis_z_coefficients) - radius_max)
            z_max = float(np.max(surface.axis_z_coefficients) + radius_max)
            epsilon = max(1.0e-9, 1.0e-10 * surface.characteristic_length)
            self._cached_bounding_box = BoundingBox(
                np.array([-radial_extent - epsilon,
                          -radial_extent - epsilon, z_min - epsilon]),
                np.array([radial_extent + epsilon,
                          radial_extent + epsilon, z_max + epsilon]),
            )
        return self._cached_bounding_box

    def translate(self, vector, inplace=False):
        raise NotImplementedError(
            'transform the coefficient data before constructing this surface')
